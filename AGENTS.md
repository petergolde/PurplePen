...
- **Header padding with a short header**: Semi's `DataGridColumnHeader` padding is `16 8`. With `ColumnHeaderHeight="24"` the 8px vertical padding pushes the header text below center — set `Padding="12,0"` (matching the cells' horizontal padding) in the `DataGrid DataGridColumnHeader` style. Read-only `DataGridTextColumn` cells get their inset from the `DataGridCellTextBlockDefaultMargin` resource (`16 0`); override it in `<DataGrid.Resources>` (`<Thickness x:Key="DataGridCellTextBlockDefaultMargin">12,0</Thickness>`) so text columns line up with the headers and the always-editable TextBox cells.
