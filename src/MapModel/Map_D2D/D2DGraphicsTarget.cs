...
namespace Map_D2D
{
    ...
    public class D2D_GraphicsTarget: IGraphicsTarget
    {
        ...
        private Stack<AntialiasMode> antiAliasStack = new Stack<AntialiasMode>();
        ...
        {
            antiAliasStack.Push(renderTarget.AntialiasMode);
            renderTarget.AntialiasMode = antiAlias ? AntialiasMode.PerPrimitive : AntialiasMode.Aliased;
        }

        // Pop anti-aliases mode.
        public void PopAntiAliasing()
        {
            renderTarget.AntialiasMode = antiAliasStack.Pop();
        }
        ...
        public void CreatePen(object penKey, CmykColor color, float width, System.Drawing.Drawing2D.LineCap caps, System.Drawing.Drawing2D.LineJoin join, float miterLimit)
        {
            if (penMap.ContainsKey(penKey))
                throw new InvalidOperationException("Key already has a pen created for it");
        ...
        public void DrawArc(object penKey, PointF center, float radius, float startAngle, float sweepAngle)
        {
            ...
            PointF ptStart = new PointF(center.X + (float)Math.Cos(startAngle * Math.PI / 180.0) * radius, center.Y + (float)Math.Sin(startAngle * Math.PI / 180.0) * radius);
            PointF ptEnd = new PointF(center.X + (float)Math.Cos(endAngle * Math.PI / 180.0) * radius, center.Y + (float)Math.Sin(endAngle * Math.PI / 180.0) * radius);
            ArcSegment segment = new ArcSegment(D2DUtil.Point(ptEnd), new D2D.SizeF(radius, radius), 0, SweepDirection.Clockwise, (sweepAngle > 180.0F) ? ArcSize.Large : ArcSize.Small);
    ...
    public static class D2DUtil
    {
        ...
        public static ColorF ToD2DColor(CmykColor color)
        {
