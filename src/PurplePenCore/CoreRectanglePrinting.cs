...
namespace PurplePen
{
    public enum CorePrintingCountKind { 
    ...
    public abstract class CoreRectanglePrinting: IPrintable
    {
        const float SPACING = 25;        // spacing of rectangles, in hundreths of an inch.

        private float boxSize;
        private CorePrintingCountKind countKind;
        private int count;

        private CoreRectanglePositioner positioner;
        private PrintingPaperSize paperSize;

        // Get all the descriptions we are going to print.
        protected abstract IPrintableRectangle[] GetRectangleList();

        public CoreRectanglePrinting(float boxSize, CorePrintingCountKind countKind, int count)
        {
        ...
        public int LayoutPages(PrintingPaperSizeWithMargins defaultPaperSizeWithMargins)
        {
            ...
            positioner = new CoreRectanglePositioner(printAreaInHundreths, boxSize / 0.254F, SPACING);

            if (countKind == CorePrintingCountKind.OneDescription) {
                positioner.LayoutMultipleDescriptions(rendererList);
            }
            else if (countKind == CorePrintingCountKind.OnePage) {
                foreach (DescriptionRenderer renderer in rendererList)
                    positioner.LayoutOneDescriptionPage(renderer);
            }
            else if (countKind == CorePrintingCountKind.DescriptionCount) {
                foreach (DescriptionRenderer renderer in rendererList) {
            ...
            else if (countKind == CorePrintingCountKind.CopyCount) {
    ...
    public class CoreRectanglePositioner
    {
        ...
        public CoreRectanglePositioner(SizeF pageSize, float boxSize, float spacing)
        {
        ...
        public void LayoutMultipleDescriptions(IPrintableRectangle[] rectangles)
        {
            ...
            rectList.Sort(delegate(IPrintableRectangle d1, IPrintableRectangle d2) {
                ...
                if (size1.Width < size2.Width)
                    ...
                    return Array.IndexOf(rectangles, d1) - Array.IndexOf(rectangles, d2);
            });
